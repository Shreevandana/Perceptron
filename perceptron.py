...
#DATA_PATH = "/Users/shreevandanakachroo/Desktop/data/adult"
...
def perceptron(train_ys, train_xs, dev_ys, dev_xs, args):
    ...
    for i in range(args.iterations):
	    for n in range(0,len(train_xs)):
	    	if train_ys[n]*np.dot(train_xs[n],weights) <=0:
...
def test_accuracy(weights, test_ys, test_xs):
    ...
    for i in range(0,len(test_ys)):
    	if np.dot(weights,test_xs[i]) < 0:
    		if test_ys[i]<0:
    			correct += 1
    	elif np.dot(weights,test_xs[i]) > 0:
    		if test_ys[i]>0:
    			correct += 1
